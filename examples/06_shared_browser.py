...
async def automatic_browser_with_brave():
    ...
    rprint("[bold cyan]=== Scenario 1: Automatic Browser + Brave ===[/bold cyan]\n")
    rprint("[dim]Browser created automatically on first use...[/dim]\n")

    async with JKAnimeScraper(headless=True, executable_path=BRAVE_PATH) as scraper:
...
async def main():
    ...
    rprint("\n[bold]Summary:[/bold]")
    rprint("  1. Automatic: Browser created on first use, closed on scraper exit")
